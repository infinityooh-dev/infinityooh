...
const CookieConsentModal = () => {
  ...
  useEffect(() => {
    ...
    if (consent) {
      const { analytics } = JSON.parse(decodeURIComponent(consent.split('=')[1]));
  ...
  return (
    <>
      {popupVisible && (
        <Dialog open={true}>
            <DialogContent position="bottom-left" hideOverlay={true} aria-describedby="cookie-consent-modal" className="rounded-lg pointer-events-auto bg-primary text-black border border-gray-300 shadow-xl p-6 w-[95vw] max-w-[500px]" >
                <DialogHeader className="space-y-3 text-left">
                  <DialogTitle className=" font-semibold text-2xl">Manage your consent preferences</DialogTitle>
                  <p className="text-sm"> We use cookies to provide you with a great experience and to help our website run effectively.</p>
                </DialogHeader>
                <div className="mt-4 flex justify-between gap-3 items-center">
                  <p className='underline mt-2 cursor-pointer text-sm' onClick={() => handleConsentModal(true)}>Manage Preferences</p>
                  <CustomButton onClick={handleAcceptAll} btnName="Accept All" className='text-sm' variant="primary"/>
                </div>
          </DialogContent>
        </Dialog>
      )}
